...
match_predictions <- read.csv(paste0(path,'final_and_third_match_predictions_2018.csv'))
...
write.csv(ranking,paste0(path,'ranking_2018.csv'),row.names = FALSE)

# Champions dataframe 
champions <- match_predictions %>% filter(stage_num == 6) %>% dplyr::select(simulation,winner)
write.csv(champions,paste0(path,'champions_2018.csv'),row.names = FALSE)