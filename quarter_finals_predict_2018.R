...
df_clean_quarter_finals <- read.csv(paste0(path,'df_test_quarter_finals_2018.csv'))

# dim(df_clean_quarter_finals) should have 8*n_simulation rows! - checked!

# Running the predictions for the simulated matches
predictions_sim <- rep(NA,nrow(df_clean_quarter_finals))

for(i in 1:nrow(df_clean_quarter_finals)) {
  predictions_sim[i] <- rpois(1,exp(coeffs[1] + 
                                      coeffs[2]*df_clean_quarter_finals$year[i] + 
                                      coeffs[3]*df_clean_quarter_finals$diff_elo[i] + 
                                      coeffs[4]*df_clean_quarter_finals$avg_goals_scored[i] + 
                                      coeffs[5]*df_clean_quarter_finals$avg_opp_goals_taken[i] + 
                                      coeffs[6]*df_clean_quarter_finals$stage_num[i] + 
                                      coeffs[7]*df_clean_quarter_finals$team_has_home_advt[i]))
}

# Why rpois only returns integers? Check why this is so!

preds_df <- na.omit(cbind(df_clean_quarter_finals,predictions_sim))
...
winner_pred_df_quarters <- winner_pred_df

# The dimension of winner_pred_df should be 4*n_simulations (checked!)
write.csv(winner_pred_df_quarters,paste0(path,'quarter_final_match_predictions_2018.csv'))
