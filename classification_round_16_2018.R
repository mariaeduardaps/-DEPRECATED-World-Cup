...
winner_pred_df <- read.csv(paste0(path,'winner_pred_df_2018.csv'))

# Rename columns 
names(group_df)[1] <- 'country'

# Duplicate to create a thousand simulations 
cols <- c('country', 'group', 'simulation') 
...
group_df <- stacked_df
# group_df[group_df$country == 'France ',]$country <- 'France'
...
# Table for image
df_clean_final %>% group_by(group,country) %>% summarize(avg_points = sum(points)/1000) %>% arrange(group,-avg_points)
...
write.csv(df_clean_final,paste0(path,'classification_round_16_2018.csv'),row.names = FALSE)