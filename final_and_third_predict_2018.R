...
df_clean_final <- read.csv(paste0(path,'df_test_final_2018.csv'))
df_clean_third <- read.csv(paste0(path,'df_test_third_place_2018.csv'))

df_clean_final <- df_clean_final %>% dplyr::select(!c(match_num_final))
df_clean_third <- df_clean_third %>% dplyr::select(!c(match_num_third_place))

# Combine those before predicting (the major difference is the stage num - final is 6 and third place is 5)
df_clean_combined <- rbind(df_clean_final,df_clean_third)

# Running the predictions for the simulated matches
predictions_sim <- rep(NA,nrow(df_clean_combined))

for(i in 1:nrow(df_clean_combined)) {
  predictions_sim[i] <- rpois(1,exp(coeffs[1] + 
                                      coeffs[2]*df_clean_combined$year[i] + 
                                      coeffs[3]*df_clean_combined$diff_elo[i] + 
                                      coeffs[4]*df_clean_combined$avg_goals_scored[i] + 
                                      coeffs[5]*df_clean_combined$avg_opp_goals_taken[i] + 
                                      coeffs[6]*df_clean_combined$stage_num[i] + 
                                      coeffs[7]*df_clean_combined$team_has_home_advt[i]))
}

# Why rpois only returns integers? Check why this is so!

preds_df <- na.omit(cbind(df_clean_combined,predictions_sim))
...
winner_pred_df_final_and_third <- winner_pred_df

write.csv(winner_pred_df_final_and_third,paste0(path,'final_and_third_match_predictions_2018.csv'))